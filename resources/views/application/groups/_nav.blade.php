        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="row align-items-center mb-2">
                    ...
                    @if($auth_user->isMemberOf($group))
                    <div class="col fs-3">
                        <button class="btn btn-orange ms-2 text-nowrap"   data-toggle="modal" data-target="#exampleModal" id="open" >
                            <i class="fas fa-comment" style="font-size: x-large;padding: 3px;"></i>{{__('Live Chat')}}
                        </button>
                    </div>
                    @endif
                </div>
...
<div class="bg-white sticky-top border py-2">
    <div class="container d-flex flex-column flex-md-row align-items-md-center justify-content-between">
        <ul class="nav mobile-horizontal">
            <li class="nav-item">
                <a class="nav-link fs-3 ps-0 {{ $page == 'about' ? 'fw-bold text-primary' : '' }}" href="{{ route('groups.about', [$group->slug,'x'=>$group->id]) }}#about">{{ __('About') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link fs-3 {{ $page == 'events' ? 'fw-bold text-primary' : '' }}" href="{{ route('groups.events', [$group->slug,'x'=>$group->id]) }}#events">{{ __('Events') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link fs-3 {{ $page == 'members' ? 'fw-bold text-primary' : '' }}" href="{{ route('groups.members', [$group->slug,'x'=>$group->id]) }}#members">{{ __('Members') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link fs-3 {{ $page == 'photos' ? 'fw-bold text-primary' : '' }}" href="{{ route('groups.photos',[$group->slug,'x'=>$group->id]) }}#photos">{{ __('Photos') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link fs-3 {{ $page == 'discussions' ? 'fw-bold text-primary' : '' }}" href="{{ route('groups.discussions',[$group->slug,'x'=>$group->id]) }}#discussions">{{ __('Discussions') }}</a>
            </li>
        </ul>
        <div>
